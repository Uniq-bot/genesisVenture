...
function MilestoneItem({ year, title, image }: { year: string; title: string; image: string }) {
  return (
    <div className="flex flex-col ml-10 mt-20 w-120">
      <div className="relative w-full h-70">
        <Image src={image} alt={title} fill />
        <div className="absolute inset-0 bg-blue-900 opacity-50" /> {/* Blue overlay */}
      </div>
      <div className="font-[PPFONT] text-blue-900">
        <h1 className="text-[170px] leading-none bg-[#e9e7e0] w-150 relative -left-15 pl-15">
          {year}
        </h1>
          ...
          return (
            <div
              ...
              className={`w-1/3 h-full flex flex-col py-10 justify-between border-l border-blue-900 ${
                isLastItem && isOdd ? "col-span-3 justify-self-center" : randomOffset
              }`}
            >
              <MilestoneItem
                year={milestone.year}
